...
def plot_performance(model_data, kmeans_data):
    ...
    title = 'Figures of Merit for Random Forest Clustering and k-Means'
    for i, dataset in enumerate(datasets):
        ...
        ax.scatter(k_data_kmeans, wcv_data_kmeans, color='r', label='k-Means')
        ax.scatter(k_data_rfc, wcv_data_rfc, color='b', label='Random Forest Clustering')
        if i == 3:
            plt.legend(loc=(-0.42,1.07),fontsize=11)
        plt.title(d[dataset])
    ...
    plt.text(-7.95,1.75,'Performance of k-Means and Random Forest Clustering', size=16)
