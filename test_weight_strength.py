...
def score_weight(weight):
    rfc = Pipeline([('rfc', RFCluster(n_forests=60,n_trees=1,n_features_to_predict=0.5,max_depth=5,outputting_weights=True,weight_extent=weight)), ('jkmeans', JKMeans(5, n_attempts=6,accepting_weights=True))])
    wcv = WCVScore(rfc)
